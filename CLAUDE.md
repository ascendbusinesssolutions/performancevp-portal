...
Organisations are provisioned by PerformanceVP (the Owner or support staff) on signature of an agreement; they do not self-register, and there is no online payment. Designated PerformanceVP support staff, and the Owner, reach a client's account only through a support session opened with a written reason; the client cannot switch this off, and every session, and everything done in it, is logged and visible to the client. There is no analyst workspace, no manual score entry and no analyst publish gate: scoring runs automatically when a campaign closes, and a client administrator reviews and releases the results.
...
- **Roles.** Owner (one PerformanceVP account, least privilege, audited break-glass). Support staff (designated PerformanceVP accounts, reaching client data only through a logged, time-limited support session the client can always see; there is no client switch). Account owner, Administrator, Executive viewer and Unit viewer on the client side. Manager respondent (signs in with a one-time email code to rate their own direct reports). Members, team leaders and leadership-team respondents are not portal users. The full matrix is in `PORTAL_BUILD_PLAN.md` Section 3.
- **Sign-in.** Account owners, administrators, viewers and staff sign in with a password; managers with a one-time email code. **Multi-factor authentication** is mandatory for the Owner, support staff, account owners and administrators, because those roles can see identified ratings. The database enforces both.
...
- **Identified ratings.** Manager ratings of named direct reports (skills, knowledge, talent bands), their evidence notes and uploaded formal performance ratings are identified and retained for traceability. They are readable only by the rating manager (own rows), administrators, the account owner and support staff under a session, never by executive or unit viewers. Administrators and staff read them only through functions that write an audit entry for every view or export. When a directory record is purged, the employee link on that person's rating rows is removed and the rows are kept. There must be no query path from a directory record to an anonymous survey response.
