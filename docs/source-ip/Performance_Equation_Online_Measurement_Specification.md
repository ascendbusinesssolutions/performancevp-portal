...
**Scoring, dates and trend.** Band counts feed the engine as the Type B input, unchanged. C3's measurement date is the formal rating date, so confidence decays from then. Where a unit's C3 source changes between cycles, the trend is annotated at that point.
