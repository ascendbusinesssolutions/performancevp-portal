...
**Unit leader.** Each unit carries a unit-leader designation, set on the units screen. At the readiness check it defaults to the single active person in the unit whose manager sits outside the unit or who has no manager; where a unit has no such person or more than one and no designation, the readiness check warns. The designation may name a member of the unit or of a grouping unit above it (the executive a unit's staff report to). The leadership team remains the people flagged in the directory (Part 2); the unit leader is expected to be among them, and the readiness check warns where they are not. The team-leader module (M-C5-TL) falls back to the unit leader where a unit has no team leaders flagged (decided 23 September 2026).
...
Trends are held against units, so unit identity has to survive a rebuild. Units carry a stable unit code. Renaming a unit keeps its history. Merging or splitting units creates new units with a recorded lineage to their predecessors; predecessor history remains viewable, and the trend line is shown as broken at that point with an annotation. A unit needs 10 or more staff, consistent with the consultancy line's definition; the minimum is not a client setting. The rule applies to the units that are measured. A unit that has units below it and fewer than 10 staff of its own is not measured on its own: the administrator either combines it for measurement with one or more of the units below it (see the next paragraph) or leaves it as a grouping unit, which is not measured, does not block the first campaign, and whose own staff are not surveyed as members of any unit, though they still act as managers and unit leaders for the people who report to them in the units below. The top of the organisation (the head and a small executive group) is usually a grouping unit. A unit with 1 to 9 people and nothing below it blocks the first campaign until it is combined for measurement, grown or retired (decided with the Milestone 4 build).

**Combining small units for measurement (decided 23 September 2026).** The directory keeps the organisation as the HRIS has it; measurement units are defined on top of it. Every unit of 10 or more is a measurement unit on its own. A unit under 10 is combined with one or more other units in the same branch of the hierarchy, and the administrator chooses which: the readiness check lists the candidates, horizontal (the unit's siblings under the same parent) and vertical, both up (the parent, and where the parent is a grouping unit, the next unit above) and down (the units directly below it, where it has any), each with its headcount and the combined total, marking any combination that would still fall short of 10. Nothing is proposed as a default and nothing is applied until the administrator chooses; for a unit with nothing below it the blocker stands until then, and for a unit with units below it the choice is between combining downward and remaining a grouping unit. Inside a combined measurement unit each constituent org unit is a team, so collective intelligence and leadership enablement are still scored for it at the team floor of four respondents. Unit context (knowledge domains, decision types, processes, systems) is defined once for the measurement unit. The leadership team is the flagged people across the constituents; the unit leader is the parent's where the combination rolls up, or chosen where siblings combine. Manager ratings are unaffected. Results, suggestions and trends attach to the measurement unit, which carries its own stable code; a constituent that later grows past 10 can be split out under the lineage rules, with the trend marked as broken. The organisation dashboard lists measurement units and shows which org units each holds. The minimum of 10 applies to the measurement unit; nothing under 10 is scored.
