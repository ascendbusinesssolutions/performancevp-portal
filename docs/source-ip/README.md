...
**These files are a snapshot.** The root masters live in the separate IP folder (`performance-equation`, alongside this repository), and this snapshot is refreshed only by copying from them. Nothing here is ever edited in place. If a document looks stale or internally inconsistent, flag it to Michael rather than correcting it here. All eleven documents were verified identical to their masters by SHA-256 on 24 September 2026, when the Cadence Master was refreshed (7.7 restated as four campaigns a year).
