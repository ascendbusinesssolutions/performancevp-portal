...
**Status.** Milestone 2 complete: Northwind and ten generated scenarios match the workbook on every mapped formula cell; every Part 4 worked example reproduces through the engine. Version 1.1.0 (Milestone 4) adds `constants.SETUP`, the setup counts the portal's readiness check applies before a first campaign; no scoring changed. Version 1.2.0 (Milestone 5) adds two optional inputs an online campaign supplies: `responses.membersPartB`, Part B submitted as its own survey, and `snapshot.audiences`, the leadership-team and team-leader audiences frozen at launch. Where they are absent nothing changes, and the Northwind parity report is identical.
...
| M-O1-LT agreement is each decision type's own modal counts over its n, as the Module Library states. The workbook computed this only from the correction of 22 September 2026 (MAXIFS over the decision type's rows); before it, an N() wrapper scored every decision from the first import row, which Northwind's single answer pattern masked. The intake never carried the defect; the parity-only adapter that reproduced it was removed with the correction. | 9 Type C Scoring C6:G17 |
| The workbook matches decision-type names as criteria in COUNTIFS, MAXIFS and AVERAGEIFS, so a name containing `*` or `?` takes in the rows of any other type it matches as a pattern. The intake matches decision types by identifier and is unaffected; the setup guidance or the next workbook pass settles the characters (IP folder DECISIONS.md item 9, parked 22 September 2026). | 4 Import Leadership I:M; 9 Type C Scoring B6:G17, B21:B32 |
...
| A Part B submitted as its own survey is screened on its own, with its own speed check and headcount line, and scored beside any Part B items on the members' rows; the workbook's single import is the case where it is absent. | Online Measurement Specification Part 2; Milestone 5 plan, D1 |
| Where the snapshot carries the frozen audiences, the leadership-team and team-leader denominators are those lists, which can hold a leader from a unit above or an unflagged unit leader taking the team-leader module; otherwise the members' flags count, as in the workbook's Setup. | Online Measurement Specification 6.1; Milestone 5 plan, D2 |
